...
function tonav() {
  ...
  document.getElementById("page-name").innerText = document.title.split(" | 今夕何夕")[0];
}
...
$.ajax({
  ...
  data: {
    key: '6AABZ-57GWL-6SYPA-EPOUG-6CZFO-CYFAC',  // 这里要写你的KEY!!!
    output: 'jsonp',
  },
  dataType: 'jsonp',
...
if (document.body.clientWidth > 992) {
  ...
  // 确保只调用一次 nekoScroll 插件
  $("#myscoll").nekoScroll({
    nekoname: 'neko1', // 给猫咪一个唯一的id
    nekoImg: '/img/KogureChika.jpg', // 【重要】猫咪的背景图片，使用你想要的图片路径
    scImg: "", // 绳子的背景图片，如果不需要可以留空
    bgcolor: 'rgb(0 0 0 / .5)', // 绳子背景色
    zoom: 0.9, // 绳子长度的缩放值
    hoverMsg: '拜托你咯~', // 鼠标悬浮在猫咪上方的对话框信息
    right: '55.6px', // 【重要】统一设置右侧距离，这个值要和CSS里的保持一致
    scroWidth: '6px', // 绳子的宽度
    borderRadius: '2em', // 绳子圆角
    z_index: 9999, // 层级
    during: 1200 // 滚动时长
  });
})
...
function createtime1() {
  var grt = new Date("06/12/2025 00:00:00"); //此处修改你的建站时间或者网站上线时间
  ...
  var ascll = [
    `欢迎来到今夕何夕の小家!`,
    `Where dreams begin their voyage`,
    `
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by 今夕何夕 %c 你正在访问今夕何夕の小家",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| 今夕何夕") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('今夕何夕的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
function createtime() {
  ...
  // 网站诞生时间
  var grt = new Date("12/06/2025 00:00:00");
  ...
  let currentTimeHtml = "";
    (currentTimeHtml =
    hnum < 18 && hnum >= 9
      ? `<img class='boardsign' src='https://img.shields.io/badge/糖果屋-科研摸鱼中-blue?style=flat-square' title='什么时候能够实现财富自由呀~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`
      : `<img class='boardsign' src='https://img.shields.io/badge/糖果屋-下班休息啦-brightgreen?style=flat-square' title='下班了就该开开心心地玩耍~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`),
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  document.getElementById("defineBg").innerText = `:root{
    --default-bg: url(/img/travel.jpg);
    --darkmode-bg: url(/img/travel.jpg);
    --mobileday-bg: url(/img/travel.jpg);
    --mobilenight-bg: url(/img/travel.jpg);
  }`;
}
// ... (后面的 function changeBg(s) 和 function setBg(s) 保持不变) ...
